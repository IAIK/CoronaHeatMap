...
const std::map<size_t, std::vector<seal::SmallModulus>> Client::default_coeff_modulus_80
          {
              ...
              */
              { 4096,{
                  0x3ffffffffd6001, 0x3ffffffffd2001,
                  0x3ffffffffbe001
                } },
              ...
              */
              { 8192,{
                  0x7ffffffec001, 0x7ffffffc8001,
                  0x7ffffffb4001, 0x7ffffff00001,
                  0x7fffffefc001, 0x7fffffecc001,
                  0x7fffffe70001
                } }
          };
...
void Client::sk_from_file(seal::SecretKey& seckey, std::shared_ptr<seal::SEALContext>& context) {
...
uint64_t Client::get_num_plaintexts() const {
  return num_plaintexts;
}

//----------------------------------------------------------------

uint64_t Client::get_num_ciphertexts() const {
  return num_ciphertexts;
}

//----------------------------------------------------------------

uint64_t Client::get_slots() const {
  return slots;
}
...
//----------------------------------------------------------------
uint64_t Client::encrypt_to_file(std::vector<uint64_t>& input) {
  std::ofstream c;
  c.open(CIPHER_FILE);
  uint64_t hw = 0;
  std::vector<uint64_t> plain;
...
bool Client::decrypt(std::vector<uint64_t>& res, std::vector<Ciphertext>& ciphs, bool cap_negative) {
  if (ciphs.size() != num_ciphertexts)
    return  false;
  res.reserve(k);
