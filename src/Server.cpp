...
Server::Server(std::shared_ptr<seal::SEALContext> con, uint64_t p_mod,
               bool bsgs)
    : context(con),
      evaluator(*context),
      encoder(*context),
...
void Server::set_pk(seal::PublicKey &pk) {
  public_key = pk;
  pk_set = true;
}
...
void Server::set_num_threads(uint64_t num) { num_threads = num; }

//----------------------------------------------------------------

void Server::diagonal(Ciphertext &state, const plain_matrix &mat) {
...
void Server::babystep_giantstep(Ciphertext &state, const plain_matrix &mat) {
  size_t matrix_dims = encoder.slot_count() >> 1;
  ...
  for (auto i = 0ULL; i < matrix_dims; i++) {
    ...
    auto l = i / bsgs_n1;
    if (l) {
      std::rotate(diag.begin(), diag.begin() + diag.size() - l * bsgs_n1,
                  diag.end());
      std::rotate(tmp.begin(), tmp.begin() + tmp.size() - l * bsgs_n1,
                  tmp.end());
    }
    for (auto &el : tmp) diag.push_back(el);
...
bool Server::compute(std::vector<seal::Ciphertext> &out) {
  if (!gk_set || !in_set) return false;

  if (!pk_set && this->noise_flooding) return false;
...
bool Server::computeWithMask(std::vector<seal::Ciphertext> &out) {
  if (!gk_set || !relin_set || !in_set) return false;

  if (!pk_set && this->noise_flooding) return false;

  return compute_internal(out, true);
}

//----------------------------------------------------------------

bool Server::compute_internal(std::vector<seal::Ciphertext> &out,
                              bool masking) {
  out.clear();
  out.resize(num_ciphertexts);
  if (input.size() != num_plaintexts) return false;

  std::cout << std::endl;
  std::cout << "    Total of " << num_ciphertexts * num_plaintexts << " matmuls"
            << std::endl;
  ...
  if (num_threads == 1) {
    ...
    for (uint64_t i = 0; i < num_ciphertexts; i++) {
      if (masking) evaluator.add_inplace(out[i], mask[i]);
      if (diff_priv) {
        std::vector<uint64_t> noise_plain(slots);
        for (uint64_t s = 0; s < slots; s++) noise_plain[s] = laplace_round();
        Plaintext noise;
        encoder.encode(noise_plain, noise);
        evaluator.add_plain_inplace(out[i], noise);
      }
      if (this->noise_flooding) {
        Ciphertext zero_with_noise;
        encrypt_zero_noise_flooding(zero_with_noise, *this->context,
                                    this->public_key, this->noise_bits);
        evaluator.add_inplace(out[i], zero_with_noise);
      }
      if (this->mod_switch) {
  ...
  std::cout << "    Spawning " << n_threads << " threads, each calculating "
            << num << " rows, total of " << num * num_ciphertexts << " each..."
            << std::flush;
  ...
  for (uint64_t i = 0; i < n_threads; i++) {
    std::thread t(runner, std::ref(t_res[i]), std::ref(input), i * num, num,
                  std::ref(*this));
  ...
  if (!VERBOSE) std::cout << "    Waiting..." << std::flush;
  ...
  std::cout << "    Combining results from threads and switch to last level..."
            << std::flush;
  for (uint64_t i = 0; i < num_ciphertexts; i++) {
    out[i] = t_res[0][i];
...
// dot product
void Server::dot(const seal::Ciphertext &in1, seal::Ciphertext &in_out) {
...
void Server::computePartBinMask(
    Ciphertext &mask, const std::vector<seal::Ciphertext> &in,
...
void Server::computeBinMask(Ciphertext &mask,
  ...
  // d vector
  std::vector<Ciphertext> in_minus_1;
  minus_1(input, in_minus_1);
  Ciphertext mask1;
...
void Server::computeMask(std::vector<Ciphertext> &mask) {
  ...
  for (uint64_t i = 0; i < num_ciphertexts; i++) {
    evaluator.multiply_plain(mask_bin, r_enc[i], mask[i]);
  }
}
...
void Server::runner(std::vector<seal::Ciphertext> &out,
                    const std::vector<seal::Ciphertext> &in,
                    uint64_t start_index, uint64_t num, Server &server) {
  // essentially big v^T * M mult
  for (uint64_t i = 0; i < server.num_ciphertexts; i++) {
    for (uint64_t j = start_index;
         j < start_index + num && j < server.num_plaintexts; j++) {
      Ciphertext r = in[j];
      plain_matrix mat;
      ...
      if (server.VERBOSE) server.increment_counter();
...
inline uint32_t Server::get_matrix_element(uint64_t row, uint64_t col) {
  ...
  // just something random for now, use real matrix in a use case
...
void Server::runner_plain(std::vector<uint64_t> &out,
                          const std::vector<uint64_t> &in, uint64_t start_index,
                          uint64_t num, Server &server) {
  for (uint64_t col = 0; col < server.k; col++) {
    uint64_t sum = 0;
    for (uint64_t row = start_index; row < start_index + num && row < server.N;
         row++) {
...
void Server::compute_plain(std::vector<uint64_t> &out,
  ...
  if (num_threads == 1) {
    ...
    if (diff_priv) {
      for (auto &el : out) el += laplace_round();
    }
    return;
  }
  ...
  for (uint64_t i = 0; i < n_threads; i++) {
    std::thread t(runner_plain, std::ref(t_res[i]), std::ref(in), i * num, num,
                  std::ref(*this));
    threads.push_back(std::move(t));
  }
  std::cout << "...done" << std::endl;
  ...
  for (uint64_t i = 0; i < k; i++) {
    ...
    if (diff_priv) out[i] += laplace_round();
  }
  std::cout << "...done" << std::endl;
}
...
void Server::keys_from_file(bool masking, bool noise_flooding_) {
  std::ifstream gk;
  gk.open(GK_FILE);
...
//----------------------------------------------------------------
void Server::activate_noise_flooding(bool activate, uint64_t noise_bits_) {
  noise_flooding = activate;
  this->noise_bits = noise_bits_;
}
